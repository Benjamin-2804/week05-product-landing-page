...
<div
    ...
    onkeydown="if(event.key==='Enter'||event.key===' ')openProductModal({{ json_encode($name) }}, {{ json_encode($price) }}, {{ json_encode($image) }}, {{ json_encode($description) }}, {{ json_encode($tag) }}, {{ json_encode($ctaHref) }})"
>
    {{-- Photo area --}}
    <div class="relative bg-gray-100 aspect-square overflow-hidden">
        ...
        <div class="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-colors duration-300 flex items-center justify-center">
            <span class="opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-white text-gray-800 text-xs font-semibold px-3 py-1.5 rounded-full shadow">
                View details
            </span>
        </div>
    </div>
    ...
    <div class="flex items-center justify-between gap-2 px-4 py-3">
        <div class="min-w-0">
            ...
            <p class="text-xs text-gray-400 mt-0.5">Click for details</p>
        </div>
        <span class="text-base font-extrabold text-orange-500 shrink-0">{{ $price }}</span>
    </div>
</div>
